...
use crate::gates::{math::MathGate, logic::LogicGate, fact::FactGate, confidence::ConfidenceGate, GateValidator};

#[derive(Debug, Clone)]
pub enum StreamEvent {
    Token { index: usize, text: String, confidence: f64, passed: bool },
    Fix { original: String, fixed: String, reason: String },
    Warning { message: String },
    Complete { total_tokens: usize, passed_count: usize, avg_confidence: f64 },
}

impl StreamEvent {
    pub fn to_sse(&self) -> String {
        match self {
            StreamEvent::Token { index, text, confidence, passed } => {
                format!("event: token\ndata: {{\"index\":{},\"text\":\"{}\",\"confidence\":{:.4},\"passed\":{}}}\n\n",
                    index, escape_json(text), confidence, passed)
            }
            StreamEvent::Fix { original, fixed, reason } => {
            ...
            StreamEvent::Complete { total_tokens, passed_count, avg_confidence } => {
    ...
    pub fn validate_token(&mut self, token: &str, context: &str) -> StreamEvent {
        ...
        for pin in self.pin_field.pins.iter() {
            ...
            let result = match pin.gate {
                ...
                Gate::Formal => crate::gates::formal::FormalGate::new().validate(&mut ball, context),
            };
...
mod tests {
    ...
    fn test_stream_validator_token() {
        ...
        match event {
            StreamEvent::Token { index, text, passed, .. } => {
                assert_eq!(index, 0);
                assert_eq!(text, "42");
                assert!(passed);
            }
