...
use super::json::{JsonValue, stringify, parse_json};
...
impl ProxyServer {
        ...
        for stream in listener.incoming() {
            match stream {
                Ok(stream) => {
                    ...
                    let calibration = std::sync::Mutex::new(
                        self.calibration.lock().unwrap().clone()
                    );
                    thread::spawn(move || {
                        let server = ProxyServer { config, calibration };
    ...
    fn handle_validate(&self, stream: &mut TcpStream, body: &str) -> CidResult<()> {
        let json = parse_json(body)
            .map_err(|e| CidError::ParseError(format!("Invalid JSON: {}", e)))?;

        let text = json.get_str("text")
            .ok_or_else(|| CidError::InvalidInput("Missing 'text' field".to_string()))?;
        let context = json.get_str("context")
            .unwrap_or("");
    ...
    fn handle_proxy(&self, stream: &mut TcpStream, body: &str) -> CidResult<()> {
        ...
        let prompt = json.get_str("prompt")
            .ok_or_else(|| CidError::InvalidInput("Missing 'prompt' field".to_string()))?;
        let model = json.get_str("model")
            .unwrap_or("gpt-4");
        let max_tokens = json.get_number("max_tokens")
        ...
        let response = JsonValue::Object(vec![
            ("response".to_string(), JsonValue::Str(result.validated_text)),
    ...
    fn handle_stream(&self, stream: &mut TcpStream, body: &str) -> CidResult<()> {
        ...
        for event in &events {
            ...
            stream.write_all(sse_data.as_bytes())
    ...
    fn handle_feedback(&self, stream: &mut TcpStream, body: &str) -> CidResult<()> {
        ...
        let raw_score = json.get_number("raw_score")
            .ok_or_else(|| CidError::InvalidInput("Missing 'raw_score' field".to_string()))?;
        let is_correct = json.get_bool("is_correct")
        ...
        let response = JsonValue::Object(vec![
            ("status".to_string(), JsonValue::Str("ok".to_string())),
            ("message".to_string(), JsonValue::Str("Feedback recorded".to_string())),
        ]);
            ...
            let (body, auth_header, auth_value) = match provider {
                Provider::OpenAI | Provider::Generic => {
                    let body = JsonValue::Object(vec![
                        ("model".to_string(), JsonValue::Str(model.to_string())),
                        ("max_tokens".to_string(), JsonValue::Number(max_tokens as f64)),
                    ...
                    (body, "Authorization", format!("Bearer {}", self.config.api_key))
                }
                Provider::Anthropic => {
                    let body = JsonValue::Object(vec![
                        ("model".to_string(), JsonValue::Str(model.to_string())),
                ...
                Provider::Gemini => {
                    let body = JsonValue::Object(vec![
                        ("contents".to_string(), JsonValue::Array(vec![
                            JsonValue::Object(vec![
                                ("parts".to_string(), JsonValue::Array(vec![
                                    JsonValue::Object(vec![
                                        ("text".to_string(), JsonValue::Str(prompt.to_string())),
                                    ]),
                                ])),
                                ("role".to_string(), JsonValue::Str("user".to_string())),
                            ]),
                        ])),
                        ("generationConfig".to_string(), JsonValue::Object(vec![
                            ("maxOutputTokens".to_string(), JsonValue::Number(max_tokens as f64)),
                        ])),
                    ]);
    ...
    fn extract_response(json: &JsonValue, provider: Provider) -> CidResult<String> {
        match provider {
            Provider::OpenAI | Provider::Generic => {
                ...
                Err(CidError::LlmError("Could not parse OpenAI response".to_string()))
            }
            Provider::Anthropic => {
                ...
                Err(CidError::LlmError("Could not parse Anthropic response".to_string()))
            }
            Provider::Gemini => {
                ...
                Err(CidError::LlmError("Could not parse Gemini response".to_string()))
            }
        }
    }
    ...
    fn send_error(stream: &mut TcpStream, status: u16, message: &str) -> CidResult<()> {
        let body = stringify(&JsonValue::Object(vec![
            ("error".to_string(), JsonValue::Str(message.to_string())),
        ]));
...
mod tests {
    ...
    fn test_proxy_config() {
        let config = ProxyConfig::new("127.0.0.1:8080", "https://api.openai.com/v1/chat/completions", "test-key");
    ...
    fn test_provider_detect_openai() {
        assert_eq!(Provider::detect("https://api.openai.com/v1/chat/completions"), Provider::OpenAI);
        assert_eq!(Provider::detect("https://my-proxy.com/chat/completions"), Provider::OpenAI);
    }

    #[cfg(feature = "proxy")]
    #[test]
    fn test_provider_detect_anthropic() {
        assert_eq!(Provider::detect("https://api.anthropic.com/v1/messages"), Provider::Anthropic);
    }

    #[cfg(feature = "proxy")]
    #[test]
    fn test_provider_detect_gemini() {
        assert_eq!(Provider::detect("https://generativelanguage.googleapis.com/v1beta/models"), Provider::Gemini);
    }

    #[cfg(feature = "proxy")]
    #[test]
    fn test_provider_detect_generic() {
        assert_eq!(Provider::detect("https://my-local-llm.com/api"), Provider::Generic);
    }
    ...
    fn test_extract_openai_response() {
        ...
        assert_eq!(ProxyServer::extract_response(&json, Provider::OpenAI).unwrap(), "Hello!");
    }
    ...
    fn test_extract_anthropic_response() {
        ...
        assert_eq!(ProxyServer::extract_response(&json, Provider::Anthropic).unwrap(), "Hello!");
    }

    #[cfg(feature = "proxy")]
    #[test]
    fn test_extract_gemini_response() {
        let json = parse_json(r#"{"candidates":[{"content":{"parts":[{"text":"Hello!"}]}}]}"#).unwrap();
        assert_eq!(ProxyServer::extract_response(&json, Provider::Gemini).unwrap(), "Hello!");
    }
}
