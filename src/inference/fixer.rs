...
impl TokenFixer {
    ...
    fn find_op(&self, expr: &str, ops: &[char]) -> Option<usize> {
        ...
        for i in (0..bytes.len()).rev() {
            match bytes[i] {
                ...
                b'(' | b'[' => depth -= 1,
                _ if depth == 0
                    && ops.contains(&(bytes[i] as char)) && i > 0 => {
                        let prev = bytes[i - 1];
                    ...
                    if captures.len() == 1 {
                        if let Ok(value) = captures[0].parse::<f64>() {
                            ...
                            if let Some(pos) = search_text.find(&original) {
                                ...
                                let overlaps = replacements.iter().any(|(s, e, _, _, _)| {
                                    abs_start < *e && abs_end > *s
                                });
                                if !overlaps {
                                    replacements.push((abs_start, abs_end, original.clone(), fixed,
    ...
    fn fix_typos(&self, text: &str) -> (String, Vec<TokenFix>) {
        ...
        for (typo, correction) in typos {
            ...
            for word in &words {
                ...
                if lower == *typo {
                    let fixed = if word.chars().next().map(|c| c.is_uppercase()).unwrap_or(false) {
    ...
    fn fix_consistency(&self, text: &str, context: &str) -> (String, Vec<TokenFix>) {
        ...
        if context_lower.contains("python") && result_lower.contains("def ")
            && !result_lower.contains("def main()") && result_lower.contains("def ") {
                let token_fix = TokenFix::new(
                    "",
                    ...
                    if pi < pattern.len() {
                        if pattern[pi] == 'd' {
                            if input[ii].is_ascii_digit() {
                                ...
                                let next = pi + 1;
                                if next < pattern.len() && (pattern[next] == '*' || pattern[next] == '+') {
                                    last_char_type = 'd';
                                } else {
                                    return None;
                                }
