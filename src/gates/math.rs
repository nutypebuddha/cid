...
impl MathGate {
    ...
    fn check_operator_validity(token: &str) -> (bool, f64) {
        ...
        let valid_start = matches!(first, b'+' | b'-' | b'*' | b'/' | b'^' | b'=' | b'(' | b')' | b'[' | b']' | b'.' | b'0'..=b'9')
            || token.bytes().all(|c| c.is_ascii_alphanumeric() || c == b'_' || c == b'.');
...
impl GateValidator for MathGate {
    fn validate(&self, ball: &mut Ball, context: &str) -> GateResult {
        ...
        let scores = [
            if balance_ok && correctness_ok { 0.95 } else { correctness_score },
            operator_score,
            domain_score,
        ];
