...
fn handle_initialize(request: &JsonRpcRequest) -> String {
    let result = JsonValue::Object(vec![
        ("protocolVersion".to_string(), JsonValue::Str("2025-03-26".to_string())),
        ...
        ("serverInfo".to_string(), JsonValue::Object(vec![
...
fn handle_resources_list(request: &JsonRpcRequest, engine: &InferenceEngine) -> String {
    ...
    let result = JsonValue::Object(vec![
        ("resources".to_string(), resources_list),
    ]);
    JsonRpcResponse::success(request.id.clone(), result).to_json()
}
...
fn handle_resources_read(request: &JsonRpcRequest, engine: &InferenceEngine) -> String {
    ...
    let uri = match uri {
        Some(u) => u,
        None => {
            return JsonRpcError::invalid_params(request.id.clone(), "Missing resource URI").to_json();
        }
    };
...
fn handle_prompts_get(request: &JsonRpcRequest) -> String {
    ...
    let name = match name {
        Some(n) => n,
        None => {
            return JsonRpcError::invalid_params(request.id.clone(), "Missing prompt name").to_json();
        }
    };
