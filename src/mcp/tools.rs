...
fn make_tool(name: &str, description: &str, params: &[(&str, &str, &str, bool)]) -> JsonValue {
    ...
    JsonValue::Object(vec![
        ...
        ("description".to_string(), JsonValue::Str(description.to_string())),
        ("inputSchema".to_string(), JsonValue::Object(vec![
...
pub fn call_tool(name: &str, arguments: &JsonValue, engine: &mut InferenceEngine) -> Result<String, String> {
    ...
    match name {
            ...
            match engine.validate(request) {
                Ok(result) => {
                    ...
                    Ok(stringify(&JsonValue::Object(vec![
                        ("validated_text".to_string(), JsonValue::Str(result.validated_text)),
                        ("original_text".to_string(), JsonValue::Str(result.original_text)),
                        ("confidence".to_string(), JsonValue::Number(result.confidence)),
        ...
        "cid_fix" => {
            ...
            let fix_list: Vec<JsonValue> = fixes.iter().map(|f| {
                JsonValue::Object(vec![
        ...
        "cid_search" => {
            ...
            let results: Vec<JsonValue> = facts.iter().map(|f| {
                JsonValue::Object(vec![
        ...
        "cid_detect_fallacies" => {
            ...
            let finding_list: Vec<JsonValue> = findings.iter().map(|f| {
                JsonValue::Object(vec![
                    ("name".to_string(), JsonValue::Str(f.name.to_string())),
                    ("category".to_string(), JsonValue::Str(f.category.to_string())),
                    ("description".to_string(), JsonValue::Str(f.description.to_string())),
                    ("indicator".to_string(), JsonValue::Str(f.indicator.to_string())),
                    ("confidence".to_string(), JsonValue::Number(f.confidence)),
                ])
            }).collect();
            ...
            Ok(stringify(&JsonValue::Object(vec![
                ...
                ("fallacy_count".to_string(), JsonValue::Number(findings.len() as f64)),
        ...
        "cid_sanity_check" => {
            ...
            match engine.sanity_check(value, category) {
                Some(result) => Ok(stringify(&JsonValue::Object(vec![
                    ...
                    ("typical_min".to_string(), JsonValue::Number(result.range.typical_min)),
                    ("typical_max".to_string(), JsonValue::Number(result.range.typical_max)),
                    ("description".to_string(), JsonValue::Str(result.range.description.to_string())),
                ]))),
        ...
        "cid_detect_biases" => {
            ...
            let finding_list: Vec<JsonValue> = findings.iter().map(|f| {
                JsonValue::Object(vec![
                    ("name".to_string(), JsonValue::Str(f.name.to_string())),
                    ("description".to_string(), JsonValue::Str(f.description.to_string())),
                    ("mitigation".to_string(), JsonValue::Str(f.mitigation.to_string())),
                    ("indicator".to_string(), JsonValue::Str(f.indicator.to_string())),
                    ("confidence".to_string(), JsonValue::Number(f.confidence)),
                ])
            }).collect();
            ...
            Ok(stringify(&JsonValue::Object(vec![
                ("text".to_string(), JsonValue::Str(text.to_string())),
                ("bias_count".to_string(), JsonValue::Number(findings.len() as f64)),
        ...
        "cid_sample" => {
            ...
            Ok(stringify(&JsonValue::Object(vec![
                ("status".to_string(), JsonValue::Str("sampling_not_available".to_string())),
                ("message".to_string(), JsonValue::Str(
                    "MCP sampling requires client support. Use cid proxy for LLM completions.".to_string()
                )),
                ("prompt_preview".to_string(), JsonValue::Str(
        ...
        "cid_kb_add" => {
            ...
            let domain_name = args.iter()
                .find(|(k, _)| k == "domain")
                .and_then(|(_, v)| v.as_str())
                .unwrap_or("alpha");
            let domain = crate::kb::facts::Domain::from_name(domain_name)
                .unwrap_or(crate::kb::facts::Domain::Alpha);
            let confidence = args.iter()
                .find(|(k, _)| k == "confidence")
                .and_then(|(_, v)| v.as_number())
                .unwrap_or(0.8);
            let fact = crate::kb::facts::Fact::dynamic(name, value, unit, source, domain, confidence);
            ...
            Ok(stringify(&JsonValue::Object(vec![
                ...
                ("domain".to_string(), JsonValue::Str(domain.description().to_string())),
                ("confidence".to_string(), JsonValue::Number(confidence)),
                ("total_facts".to_string(), JsonValue::Number(engine.kb_count() as f64)),
            ])))
        }
        "cid_kb_context" => {
            let query = get_required_str(args, "query")?;
            ...
            let domain_filtered: Vec<&crate::kb::facts::Fact> = results.into_iter()
                .filter(|f| f.domain == domain)
                .take(max_results)
                .collect();
            let fact_list: Vec<JsonValue> = domain_filtered.iter().map(|f| {
                JsonValue::Object(vec![
                    ("name".to_string(), JsonValue::Str(f.name.clone())),
                    ("value".to_string(), JsonValue::Number(f.value)),
                    ("unit".to_string(), JsonValue::Str(f.unit.clone())),
                    ("source".to_string(), JsonValue::Str(f.source.clone())),
                    ("domain".to_string(), JsonValue::Str(f.domain.description().to_string())),
                    ("confidence".to_string(), JsonValue::Number(f.confidence)),
                ])
            }).collect();
            Ok(stringify(&JsonValue::Object(vec![
                ("query".to_string(), JsonValue::Str(query.to_string())),
                ("domain".to_string(), JsonValue::Str(domain.description().to_string())),
                ("count".to_string(), JsonValue::Number(fact_list.len() as f64)),
                ("results".to_string(), JsonValue::Array(fact_list)),
            ])))
        }
        "cid_kb_recent" => {
            let within_secs = args.iter()
                ...
                None => {
                    ...
                    match crate::tanto::evaluate_nl(expr, &env) {
                        Some(val) => Ok(stringify(&JsonValue::Object(vec![
                            ("expression".to_string(), JsonValue::Str(expr.to_string())),
                            ("rational".to_string(), JsonValue::Str("(not rational)".to_string())),
        ...
        "cid_tanto_eval" => {
            ...
            match crate::tanto::evaluate_nl(expr, &env) {
                Some(result) => Ok(stringify(&JsonValue::Object(vec![
                    ...
                    ("formatted".to_string(), JsonValue::Str(crate::tanto::math::format_f64(result))),
                ]))),
                None => Err(format!("Cannot evaluate: '{}'", expr)),
            }
        }
        "cid_tanto_convert" => {
            ...
            match crate::tanto::convert::convert(&convert_args) {
                Some(result) => Ok(stringify(&JsonValue::Object(vec![
                    ...
                    ("formatted".to_string(), JsonValue::Str(
                        format!("{} {} = {} {}", value, result.from, crate::tanto::math::format_f64(result.value), result.to)
                    )),
                ]))),
        ...
        "cid_tanto_formula" => {
            ...
            match crate::tanto::formulas::compute_formula(&formula_args) {
                Some(result) => Ok(stringify(&JsonValue::Object(vec![
                    ...
                    ("args".to_string(), JsonValue::Array(
                        result.args.iter().map(|&a| JsonValue::Number(a)).collect()
                    )),
                    ("result".to_string(), JsonValue::Number(result.result)),
                    ("formatted".to_string(), JsonValue::Str(crate::tanto::math::format_f64(result.result))),
                ]))),
                None => Err(format!("Unknown formula or bad args: {} {}", name, args_str)),
            }
        }
        "cid_tanto_solve" => {
            ...
            match crate::tanto::solver::solve(&solve_args) {
                ...
                None => Err(format!("Unknown solver or bad args: {} {}", solver, args_str)),
            }
        }
        "cid_tanto_think" => {
        ...
        "cid_tanto_estimate" => {
            ...
            match crate::tanto::sanity::estimate(&value.to_string()) {
                Some(result) => Ok(stringify(&JsonValue::Object(vec![
                    ("value".to_string(), JsonValue::Number(result.val)),
                    ("order_of_magnitude".to_string(), JsonValue::Number(result.order as f64)),
                    ("context".to_string(), JsonValue::Str(result.context)),
                    ("below_value".to_string(), JsonValue::Number(result.below_val)),
                    ("below_name".to_string(), JsonValue::Str(result.below_name)),
                    ("above_value".to_string(), JsonValue::Number(result.above_val)),
        ...
        "cid_tanto_verify" => {
            ...
            match crate::tanto::verify::verify(&verify_args, &env) {
                Some(result) => Ok(stringify(&JsonValue::Object(vec![
                    ...
                    ("status".to_string(), JsonValue::Str(result.status.to_string())),
                ]))),
        ...
        "cid_tanto_test" => {
            ...
            let test_results: Vec<JsonValue> = results.iter().map(|(name, ok)| {
                JsonValue::Object(vec![
                    ("name".to_string(), JsonValue::Str(name.to_string())),
