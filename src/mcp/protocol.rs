...
impl JsonRpcRequest {
    pub fn parse(input: &str) -> Option<Self> {
        ...
        let method = obj.iter().find(|(k, _)| k == "method")?.1.as_str()?.to_string();
        let id = obj.iter().find(|(k, _)| k == "id").map(|(_, v)| v.clone());
        let params = obj.iter().find(|(k, _)| k == "params")
            .map(|(_, v)| v.clone())
...
impl JsonRpcError {
    pub fn new(id: Option<JsonValue>, code: i64, message: &str) -> Self {
        JsonRpcError { id, code, message: message.to_string(), data: None }
    }
...
pub fn tool_result(id: Option<JsonValue>, text: &str) -> String {
    let resp = JsonRpcResponse::success(
        id,
        JsonValue::Object(vec![
            ("content".to_string(), JsonValue::Array(vec![content_text(text)])),
