...
pub fn verify(args: &str, env: &TantoEnv) -> Option<VerifyResult> {
    ...
    let rest = if args.len() > 6 { trim(&args[6..]) } else { return None; };
    let (expected_str, expr_rest) = split_token(rest)?;
    let expected = parse_f64(expected_str)?;
    let expr = trim(expr_rest);
    if expr.is_empty() { return None; }

    let computed = eval_math(expr, env)
        .or_else(|| eval_op_format(expr, env))?;
    ...
    Some(VerifyResult { status, expected, computed, diff })
}
...
pub fn run_self_test() -> Vec<(&'static str, bool)> {
    ...
    results.push(("add 2 3 | mul 6 _", test_pipeline(b"div 1 3 | mul 6 _", 2.0)));
...
fn test_expr_nl(expr: &[u8], expected: f64) -> bool {
    ...
    let computed = eval_math(&processed, &env)
        .or_else(|| eval_op_format(&processed, &env));
    match computed {
        Some(v) => (expected - v).abs() < 1e-6,
        None => false,
    }
