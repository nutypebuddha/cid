...
pub fn list_solvers() -> Vec<(&'static str, &'static str)> {
    vec![
        ...
        // New solvers (Phase 2)
        ("stats <v1 v2 ...>", "Statistics: mean, median, std_dev, variance"),
        ("kinematics <v0 v t>", "Kinematics: a, displacement, avg_v"),
        ("thermo <P V n T>", "Ideal gas law (PV=nRT), internal energy"),
...
fn solve_orbit(args: &str) -> Option<String> {
    ...
    let grav = mu / (r * r);
    let cent = v * v / r;
    let verify = if cent.to_bits() == grav.to_bits() { "OK (exact)" } else { "CLOSE (float rounding)" };
    Some(format!(
        "=== ORBITAL SOLUTION ===
  v (velocity)    = {} m/s
  T (period)      = {} s
