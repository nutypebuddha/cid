...
use cid::core::pin::{PinField, Gate};
...
use cid::gates::{math::MathGate, logic::LogicGate, fact::FactGate, confidence::ConfidenceGate, formal::FormalGate};
use cid::gates::GateValidator;
use cid::kb::facts::KnowledgeBase;
use cid::output::trace::TraceLog;
use cid::inference::{InferenceEngine, ValidationRequest, ProxyConfig, ProxyServer};
...
impl CIDDevice {
    ...
    fn show_pins(&self) -> String {
        ...
        for pin in &self.pin_field.pins {
            ...
            output.push_str(&format!("  {:?}: threshold={:.2}, cost={:.6}, status={}\n", 
    ...
    if args.len() > 1 {
        match args[1].as_str() {
            ...
            "validate" => {
                ...
                match engine.validate(request) {
                    Ok(result) => {
                        ...
                        println!("{}", stringify(&JsonValue::Object(vec![
            ...
            "compress" => {
                ...
                println!("Saved: {} tokens ({:.1}%)", stats.saved_tokens, stats.saved_percent);
                return;
            }
            "score" => {
                if args.len() < 3 {
...
fn process_line(line: &str, device: &mut CIDDevice) -> String {
    ...
    match cmd {
        ...
        "version" | "v" => "CID v0.1.0 -- Calibrated Inference Device + Tanto Compute Engine".to_string(),
        ...
        "beam" => {
            ...
            output.push_str(&cid::output::format::format_validation_summary(&candidates, selected.as_ref()));
            output.push('\n');
            ...
            match parts[1] {
                ...
                "lookup" => {
                    ...
                    match device.kb.lookup(parts[2]) {
                        Some(fact) => format!("{} = {} {} [{}]", fact.name, fact.value, fact.unit, fact.source),
        ...
        "tanto" => {
            ...
            let subcmd = parts[1];
            let rest = if parts.len() > 2 { line[line.find(subcmd).unwrap_or(0) + subcmd.len() + 1..].trim() } else { "" };
            
            let env = &mut cid::tanto::TantoEnv::new();
            
            match subcmd {
                "eval" => {
                    match cid::tanto::evaluate_nl(rest, env) {
                        Some(val) => format!("= {}", cid::tanto::math::format_f64(val)),
                        None => format!("Error: cannot evaluate '{}'", rest),
                    }
                }
                "convert" => {
                    match cid::tanto::convert::convert(rest) {
                        Some(cr) => format!("= {} {} (from {} to {})", 
                            cid::tanto::math::format_f64(cr.value), cr.to, cr.from, cr.to),
                        None => format!("Error: unknown conversion '{}'", rest),
                    }
                ...
                "solve" => {
                    match cid::tanto::solver::solve(rest) {
                        Some(sr) => sr.output,
                        None => format!("Error: unknown solver '{}'", rest),
                    }
                }
                "think" => {
                    match cid::tanto::thinking::think(rest) {
                        Some(tr) => tr.body,
                        None => format!("Error: unknown framework '{}'", rest),
                    }
                }
                "check" => {
                    match cid::tanto::sanity::check(rest) {
                        Some(cr) => {
                            let status = if cr.val < cr.min { "BELOW RANGE" } else if cr.val > cr.max { "ABOVE RANGE" } else { "OK" };
                            format!("{} {} [range: {} - {}, typical: {}, status: {}]",
                                cid::tanto::math::format_f64(cr.val), cr.unit,
                ...
                "estimate" => {
                    match cid::tanto::sanity::estimate(rest) {
                        Some(er) => format!("{} ~ 10^{} ({})", cid::tanto::math::format_f64(er.val), er.order, er.context),
                        None => format!("Error: bad number '{}'", rest),
                    }
                }
                "pipeline" => {
                    match cid::tanto::pipeline::evaluate_pipeline(rest, env) {
                        Some(val) => format!("= {}", cid::tanto::math::format_f64(val)),
                        None => format!("Error: cannot evaluate pipeline '{}'", rest),
                    }
                }
                "rational" => {
                    match cid::tanto::rational::eval_rational(rest, env) {
                        Some(rat) => format!("= {}  (decimal: {}, mixed: {})", rat.format(), rat.to_f64(), rat.format_mixed()),
                        None => format!("Error: cannot evaluate '{}' as rational", rest),
                    }
                }
                "verify" => {
                    match cid::tanto::verify::verify(rest, env) {
                        Some(vr) => format!("{} (expected={}, computed={}, diff={})",
                            vr.status, cid::tanto::math::format_f64(vr.expected),
                            cid::tanto::math::format_f64(vr.computed), cid::tanto::math::format_f64(vr.diff)),
                        None => format!("Error: verify needs: verify <expected> <expr>"),
                ...
                "test" => {
                    ...
                    out.push_str(&format!("=== SUMMARY: {} correctness + {} determinism tests ===\n", pass, det_pass));
                    out
                }
                _ => format!("Unknown tanto subcommand: {}. Try 'help'.", subcmd),
            }
